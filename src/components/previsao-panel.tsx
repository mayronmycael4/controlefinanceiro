    ...
    <Card>
      ...
      <CardContent className="grid gap-5 md:grid-cols-2">
        ...
        <div>
          ...
          <div data-sensitive="true" className="text-3xl font-bold">{formatBRL(despesaPrevista)}</div>

          <Progress value={pctPago} className="mt-3" />
          <div className="mt-2 flex flex-wrap gap-x-4 gap-y-1 text-xs">
            <span className="flex items-center gap-1 text-emerald-600 dark:text-emerald-500">
              <CheckCircle2 className="size-3.5" />
              Já paguei <span data-sensitive="true">{formatBRL(despesaPaga)}</span>
            </span>
            <span className="flex items-center gap-1 text-amber-600 dark:text-amber-500">
              <Clock className="size-3.5" />
              Falta pagar <span data-sensitive="true">{formatBRL(despesaPendente)}</span>
            </span>
          </div>
  ...
  return (
    <div className="flex items-center justify-between gap-2">
      <div>
        ...
        {sub && <div data-sensitive="true" className="text-xs text-muted-foreground">{sub}</div>}
      </div>
      <div data-sensitive="true" className={cn("font-mono font-bold", forte ? "text-lg" : "text-base", cor)}>
        {formatBRL(valor)}
      </div>
    </div>
  );
