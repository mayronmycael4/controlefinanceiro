...
export function FiisView({ fiis }: { fiis: Fii[] }) {
  ...
  return <>
    ...
    <div data-privacy-cards="true" data-fii-mode={mode}>{mode === "table" ? <TableView fiis={fiis} /> : mode === "list" ? <ListView fiis={fiis} /> : <div className={mode === "minimal" ? "grid gap-3 sm:grid-cols-2 lg:grid-cols-4" : "grid gap-4 sm:grid-cols-2 lg:grid-cols-3"}>{fiis.map((f) => mode === "minimal" ? <MinimalCard key={f.id} f={f} /> : <NormalCard key={f.id} f={f} />)}</div>}</div>
  </>;
}

function MinimalCard({ f }: { f: Fii }) { return <Link href={`/fiis/${f.id}`}><Card className="transition hover:border-foreground/30"><CardContent className="grid gap-2 p-4"><div className="flex items-center justify-between"><div><span className="font-bold">{f.ticker}</span><p className="truncate text-xs text-muted-foreground">{f.name}</p></div><span className={f.lucroTotal >= 0 ? "text-emerald-600" : "text-red-600"}>{f.rentabilidadeTotalPct.toFixed(2)}%</span></div><div className="flex justify-between text-sm"><span className="text-muted-foreground">Cotas</span><span data-sensitive="true">{f.quantidade}</span></div><div className="flex justify-between text-sm"><span className="text-muted-foreground">Cotação</span><span>{formatBRL(f.currentPrice)}</span></div><div className="flex justify-between border-t pt-2 text-sm"><span className="text-muted-foreground">Valor atual</span><strong data-sensitive="true">{formatBRL(f.valorAtual)}</strong></div></CardContent></Card></Link>; }
...
function ListView({ fiis }: { fiis: Fii[] }) { return <div className="grid gap-2">{fiis.map((f) => <Link key={f.id} href={`/fiis/${f.id}`}><Card className="transition hover:border-foreground/30"><CardContent className="flex flex-wrap items-center gap-4 p-4"><div className="min-w-28"><p className="font-bold">{f.ticker}</p><p className="truncate text-xs text-muted-foreground">{f.name}</p></div><div><p className="text-xs text-muted-foreground">Cotas</p><p data-sensitive="true">{f.quantidade}</p></div><div><p className="text-xs text-muted-foreground">Cotação</p><p>{formatBRL(f.currentPrice)}</p></div><div><p className="text-xs text-muted-foreground">Valor</p><p data-sensitive="true">{formatBRL(f.valorAtual)}</p></div><div className="ml-auto"><Badge variant={f.lucroTotal >= 0 ? "secondary" : "destructive"}>{formatBRL(f.lucroTotal)} · {f.rentabilidadeTotalPct.toFixed(2)}%</Badge></div></CardContent></Card></Link>)}</div>; }
