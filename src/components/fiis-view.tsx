...
export function FiisView({ fiis }: { fiis: Fii[] }) {
  ...
  return <>
    ...
    {mode === "table" ? <TableView fiis={fiis} /> : mode === "list" ? <ListView fiis={fiis} /> : <div className={mode === "minimal" ? "grid gap-3 sm:grid-cols-2 lg:grid-cols-4" : "grid gap-4 sm:grid-cols-2 lg:grid-cols-3"}>{fiis.map((f) => mode === "minimal" ? <MinimalCard key={f.id} f={f} /> : <NormalCard key={f.id} f={f} />)}</div>}
  </>;
}
